...
\usetikzlibrary{positioning,matrix,fit,decorations.pathreplacing, calc}
...
\begin{tikzpicture}[remember picture]
	...
	% --------------- start PIO2 ---------------
	% host description
	\node[host description, below=4cm of router-description] (router-description2) {%
		\begin{tcolorbox}[
			adjusted title=RIO,
			host description box
		]			
			\ip{10.1.1.35}{subnet-home}
	...
	\node[
		below=0em of router-description2.south west,
		anchor=north west
	] (laptop-services2) {
		\begin{tcboxedraster}[services raster]{services box}
			\tcbox{\service{}{Generic IO}{}}
		\end{tcboxedraster}
	};
	...
	% network interfaces
	\node[anchor=north] at (router-services2.north east -| router-description2.east) (router-interface2) {
		\begin{tcbitemize}[interfaces raster]
			\tcbitem[remember as=router-eth1] eth0..1\hfill\faIcon{ethernet}
		\end{tcbitemize}
	...
	\begin{pgfonlayer}{hostboxes}
		\node[
			host box,
			fit=(router-description2.center) (router-services2.south) (router-services2.west) (router-interface2.south)
		] (router-host-box2) {};
	\end{pgfonlayer}
	% --------------- end router ---------------
	...
	\node[
		...
		\begin{tcboxedraster}[services raster]{services box}
			\tcbox[enhanced, remember as=raspberrypi-pihole]{\service{}{CORE}}
		\end{tcboxedraster}
	};
	...
	\begin{scope}[ethernet]
		...
		\draw[subnet-home] (raspberrypi-eth1.west) -- ++(-1,0) |- (router-eth1.east);	
	\end{scope}

	% --------------- end network connections ---------------

	\draw[black,thick] ($(router-description.north west)+(-1,0.3)$)  rectangle node[above=5cm of router-description]{CAB-CORE-02
	}($(router-description2.south east)+(1.5,-2)$);
\end{tikzpicture}
